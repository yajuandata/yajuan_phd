...
saccade_ss <- function(dt_path, loss_info, dt_mianji1, dt_mianji2) {
  ...
  dt_saccade_summary <- bind_rows(dt_saccade_summary1, dt_saccade_summary2) %>%
    drop_na(type, AOI) %>%
    select(participants, type, pic, AOI_type, AOI, saccade_duration, saccade_ampl, saccade_pv, saccade_ag, saccade_per_mianji, regression_per_mianji)
  # 返回数据
...
# 定义处理结果的保存路径
save_path <- "../yajuan_data/statics/sacc/sacc_02.csv"
