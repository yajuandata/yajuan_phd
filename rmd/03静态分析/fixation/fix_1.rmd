...
fixation_ss <- function(dt_path, loss_path, words_path) {
...
fixation_ss_all <- function(path, loss_path, words_path) {
  # 读取文件夹下的全部数据文件
  dt_path <- list.files(path, full.names = TRUE)

  # 对全部数据文件进行处理
  dt_fixation_summary <- dt_path %>%
    future_map_dfr(fixation_ss, loss_path = loss_path, words_path = words_path)
...
# 对文件夹下的全部数据文件进行处理
dt_fixation_summary <- fixation_ss_all(path, loss_path, words_path) %>%
