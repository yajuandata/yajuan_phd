...
fixation_ss <- function(dt_path, loss_path, mianji_path) {
  # 读取loss数据，字数数据，fixation数据
  loss_info <- fread(loss_path)

  dt_mianji <- read_csv(mianji_path)
  ...
  dt_aoi_s2 <- dt_aoi %>%
    ...
    left_join(dt_mianji2, by = c("pic", "AOI")) %>%
  ...
  # 删除其余中间数据
  rm(dt_mianji, dt_mianji2, dt_a, dt_aoi, loss_info, dt_a_s2, dt_aoi_s2)
...
fixation_ss_all <- function(path, loss_path, mianji_path) {
  # 读取文件夹下的全部数据文件
  dt_path <- list.files(path, full.names = TRUE)

  # 对全部数据文件进行处理
  dt_fixation_summary <- dt_path %>%
    future_map_dfr(fixation_ss, loss_path = loss_path, mianji_path = mianji_path)
...
# 对文件夹下的全部数据文件进行处理
dt_fixation_summary <- fixation_ss_all(path, loss_path, mianji_path) %>%
  arrange(participants, type, AOI) %>%
  select(participants, type, AOI, fixation_duration, fixation_ps, fixation_per_word)
