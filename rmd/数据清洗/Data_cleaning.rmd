...
track_loss_rate_bin <- function(dt) {
    ...
    track_loss_rate_p <- dt2 %>% group_by(block, pic) %>% summarise(track_loss_rate_pic = mean(track_loss))

    # 计算每个block下的丢失率，即track_loss列的均值
    track_loss_rate_b <- dt2 %>% group_by(block) %>% summarise(track_loss_rate_block = mean(track_loss))

    # 合并两种丢失率,并新增一列dt2的整体丢失率
    track_loss_rate_t <- track_loss_rate_p %>% left_join(track_loss_rate_b, by = "block") %>% mutate(track_loss_rate_total = mean(dt2$track_loss), participants = dt2$participants[1])
...
```{r, echo=FALSE, message=FALSE}
# 输入目录信息
dir <- "/Users/placenameday/R study/yajuan_data/out/raw2"
