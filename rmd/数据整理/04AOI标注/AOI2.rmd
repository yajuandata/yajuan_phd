...
aoi_cacu1 <- function(x, a, b) {
  ...
  fixation2 <- b %>%
    mutate(!!xs :=case_when(between(sxp, aoi1$X1, aoi1$X2)&between(syp, aoi1$Y1, aoi1$Y3)~1,
                            .default = 0)) %>%
    mutate(!!xe :=case_when(between(exp, aoi1$X1, aoi1$X2)&between(eyp, aoi1$Y1, aoi1$Y3)~1,
                            .default = 0))
}
...
aoi_cacu3 <- function(x, y) {
  ...
  # 输出数据
  write_csv(fixation2, paste0(y, ".aoi.csv"))
}
...
# 批量处理数据
aoi_cacu5("/Users/placenameday/R study/yajuan_data/out/sacc2")
# 恢复单线程处理
plan(sequential)
